...
v3 변경점 (sentiment_bot 연동):
  - 발송한 요약을 digests/YYYY-MM-DD.md 로도 저장 (하루 여러 회 실행 시 최신이 위로)
  - sentiment_bot이 이 파일 10일치를 읽어 '리서치 다이제스트 톤' 지표로 채점
  - telegram_digest.yml 커밋 스텝이 digests/ 를 함께 커밋 (v3 yml 참조)
...
def save_digest(summary):
    """sentiment_bot 연동 (v3): 발송한 요약을 digests/YYYY-MM-DD.md 로 저장.
    하루 4회 실행되므로 같은 날짜 파일에 최신 실행분을 맨 위에 붙임(prepend) —
    sentiment_bot이 일자별 앞부분부터 읽으므로 최신 요약이 우선 반영됨."""
    try:
